...
class Camera:
    ...
    def to(self, device: Union[torch.device, Literal["cuda", "cpu"]]):
        # TODO unsupport cuda:rank_id
        ...
        return Camera(
            ...
            width=self.width,
            height=self.height,
            c2w=self.c2w.to(device) if isinstance(self.c2w, torch.Tensor) else None,
            FovX=self.FovX,
            FovY=self.FovY,
            R=self.R.to(device) if isinstance(self.R, torch.Tensor) else None,
            T=self.T.to(device) if isinstance(self.T, torch.Tensor) else None,
            zfar=self.zfar,
            znear=self.znear,
            trans=self.trans.to(device) if isinstance(self.trans, torch.Tensor) else None,
            scale=self.scale,
            world_view_transform=self.world_view_transform.to(device)
            if isinstance(self.world_view_transform, torch.Tensor)
            else None,
            ...
            if isinstance(self.full_proj_transform, torch.Tensor)
            else None,
            camera_center=self.camera_center.to(device) if isinstance(self.camera_center, torch.Tensor) else None,
        )
...
def get_interpolated_k(k_a, k_b, steps: int = 10):
    """
...
def focus_of_attention(poses, initial_focus: torch.Tensor) -> torch.Tensor:
